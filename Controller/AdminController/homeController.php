...
//SELECT DAYOFWEEK(c.data_com) as ziua, SUM(TRUNCATE(p.pret_buc*pc.cantitate,2)) as totalvanzari from produse p 
//join produse_comandate pc on pc.id_produs = p.id 
//join comenzi c on c.id = pc.id_comanda where WEEK(c.data_com)=WEEK(CURRENT_DATE) 
//GROUP BY ZIUA--suma totala castigata intr-o zi: dayofweek(data_com) reprezinta ziua din saptamana
    ...
    function getVanzari()
    {
        ...
        $conn = $db->getConnection();
        try{
            $stmt = $conn->query('SELECT DAYOFWEEK(c.data_com) as ziua, SUM(TRUNCATE(p.pret_buc*pc.cantitate,2)) as totalvanzari from produse p 
                                    join produse_comandate pc on pc.id_produs = p.id 
                                    join comenzi c on c.id = pc.id_comanda where WEEK(c.data_com)=WEEK(CURRENT_DATE) 
                                    GROUP BY ZIUA;');
            $stmt->execute();
            ...
            if ($stmt == FALSE) {
                $returnMsg->ErrorMessage("Eraore necunoscuta la preluarea vanzarilor din baza de date!", -1);
            } else {
                $totalVanzariPeZi = $stmt->fetchAll(PDO::FETCH_ASSOC);
        ...
        $returnData = new Transporter($totalVanzariPeZi,$returnMsg);
        return $returnData;
    }
...
if(isset($_GET['action']))
{
    ...
    switch ($_GET['action']){
        ...
        case 'get_vanzari':
            ...
            echo json_encode($homeCtrl->getVanzari());
            break;
